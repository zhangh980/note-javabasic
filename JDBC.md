...
1. JDBC概述
   ...
   2. 通用的SQL数据库存取和操作的公共接口：使用这套类库可以以一种标准的方法、方便地访问数据库资源
   3. JDBC驱动
      1. JDBC接口的实现类
      2. 由数据库厂商提供
      3. JDBC的编程，编写代码阶段，可以没有驱动，但是要运行JDBC程序必须要有驱动，否则接口的方法都是抽象方法，没有方法体，无法执行，而且接口也没有办法创建对象，要具体的实现类才能创建对象，实现类对象往往通过工厂等一些方法获取
   4. 面向接口编程的思想
      1. ...
      2. JDBC是sun公司提供一套用于数据库操作的接口，java程序员只需要面向这套接口编程即可。不同的数据库厂商，需要针对这套接口，提供不同实现。不同的实现的集合，即为不同数据库的驱动。————面向接口编程
   5. 使用jdbc以后的好处
      1. 程序员不需要关注不同数据库的细节
      2. 使得编写的程序具有更好的可移植性
2. 如何获取数据库的连接
   1. 体会一下五种连接为何要一次次的迭代
   2. 如何连接
      1. 需要：IP，端口号，用户名，密码，数据库名
      2. 其中IP地址和端口号，以及数据库名，通过URL进行传递
      3. URL的规范：`jdbc:<子协议>:<子名称>`
         1. mysql的url标准：jdbc:mysql://localhost:3306/数据库名
         2. oracle的url标准：jdbc:oracle:thin:@localhost:1521:数据库名
         3. sqlserver的url标准：                jdbc:sqlserver://localhost:1433:DatabaseName=数据库名
3. 体会Statement的弊端
   1. 必要情况下，需要拼串
   2. 存在SQL注入的问题
4. PreparedStatement实现CRUD操作
   ...
   4. 通用的查询操作（ORM编程思想）
      ...
      2. 一个java类 对应 数据库中的一个表
      3. java类的一个属性 对应 数据表中的一个字段(列)
      4. java类的一个对象 对应 数据表的一行数据
5. PreparedStatement其他操作
   ...
   2. 批量插入数据
      ...
      2. 添加参数：?rewriteBatchedStatements=true
      3. 使用mysql更新的驱动
      4. 操作完，一次性提交
6. 数据库事务
   1. 什么是事务？
   2. 事务处理的原则？要么都提交，要么都不提交
   3. 改进原有的CRUD操作
   4. 事务操作的步骤
        ```java
...
7. 提供通用的DAO\<T\>
    ...
    class CustomerDAOImpl extends BaseDAO<Customer> implements CustomerDAO
    ```
8. 如何使用数据库连接池连接数据库
   1. 连接池的好处？
   ...
   3. DBCP数据库连接池
   4. druid数据库连接池
9.  如何使用dbutils.jar中的QueryRunner实现CRUD操作

### Web技术概览

![](image/JDBC/2021-11-08-00-10-22.png)
![](image/JDBC/2021-11-08-00-10-31.png)
...
![](image/JDBC/1555575760234.png)

***

- 有了JDBC，Java程序访问数据库时是这样的：

![](image/JDBC/1555575981203.png)

***

- 总结如下：

![](image/JDBC/1566741692804.png)
...
- JDBC API 是一系列的接口，它使得应用程序能够进行数据库联接，执行SQL语句，并且得到返回结果。
	![](image/JDBC/2021-11-08-18-28-53.png)

- JDBC程序访问数据库步骤
	![](image/JDBC/1565969323908.png)
...
- MySQL的JDBC下载地址：https://dev.mysql.com/downloads/connector/j/
	![](image/JDBC/2021-11-09-14-39-08.png)
	![](image/JDBC/2021-11-09-14-43-11.png)

- 将上述jar包拷贝到Java工程的一个目录中，习惯上新建一个lib文件夹。
	![](image/JDBC/1566134718955.png)

- 在驱动jar上右键-->Build Path-->Add to Build Path
	![](image/JDBC/1566134781682.png)

- 注意：如果是Dynamic Web Project（动态的web项目）话，则是把驱动jar放到WebContent（有的开发工具叫WebRoot）目录中的WEB-INF目录中的lib目录下即可
	
	![](image/JDBC/1566135290460.png)
...
## 第3章：使用PreparedStatement实现CRUD（create 添加数据、read 读取数据、update 修改数据、delete 删除数据）操作
...
连接数据库、操作表的步骤  
> &emsp;注册驱动 (只做一次)  
> &emsp;建立连接(Connection)   
> &emsp;创建执行SQL的语句(Statement)  
> &emsp;执行语句  
> &emsp;处理执行结果(ResultSet)  
> &emsp;释放资源
...
![](image/JDBC/1555580275036.png)


**练习题2：创立数据库表 examstudent，表结构如下：**

![](image/JDBC/1555580735377.png)

向数据表中添加如下数据：

![](image/JDBC/1555580763636.png)

**代码实现1：插入一个新的 student 信息**
...
![](image/JDBC/1555580937490.png)

**代码实现3：完成学生信息的删除功能**

![](image/JDBC/1555580965019.png)
...
rs = ps.executeQuery();
if(rs.next()){
	Integer id = rs.getInt(1); // rs.getInt("id");
    String name = rs.getString(2); // rs.getString("name");
	String email = rs.getString(3); // rs.getString("email");
    Date birth = rs.getDate(4); // rs.getDate("birth");
...
哪些操作会导致数据的自动提交？
> DDL操作一旦执行，都会自动提交。  
> &emsp;set autocommit = false 对DDL操作失效  
> DML默认情况下，一旦执行，就会自动提交。  
> &emsp;我们可以通过 set autocommit = false 的方式取消DML操作的自动提交。  
> 默认在关闭连接时，会自动的提交数据
...
![](image/JDBC/2021-11-08-21-32-41.png)

![](image/JDBC/2021-11-08-21-32-51.png)
...
![](image/JDBC/1566726681515.png)

- 层次结构：

![](image/JDBC/1566745811244.png)
...
![](image/JDBC/1555593464033.png)

- **工作原理：**

![](image/JDBC/1555593598606.png)
...
// 此处构造方法的参数与<named-config name="helloc3p0">中name属性值必须相同
...
- ![](image/JDBC/2021-11-08-21-41-02.png)
- 配置属性说明

| 属性                       | 默认值 | 说明                                                                                                                                              |
| -------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| initialSize                | 0      | 连接池启动时创建的初始化连接数量                                                                                                                  |
| maxActive                  | 8      | 连接池中可同时连接的最大的连接数。 高峰单机器在20并发左右，自己根据应用场景定                                                                     |
...
下载链接1：https://repo1.maven.org/maven2/com/alibaba/druid/
...
![](image/JDBC/1555595163263.png)

![](image/JDBC/1555595198644.png)


![](image/JDBC/2021-11-08-21-43-00.png)
